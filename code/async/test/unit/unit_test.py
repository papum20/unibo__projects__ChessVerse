...
class TestChessSocketIO(IsolatedAsyncioTestCase):
    ...
    async def test_on_resign(self, _, __, ___):
        ...
        self.sio.emit.assert_called_with("error", {"cause": "Game not found", "fatal": True}, room=self.pve)
        await self.game_handler.on_start(self.pve, {"rank": 70, "depth": 1, "time": 300, "type": GameType.PVE})
        await self.game_handler.on_resign(self.pve, {"type": GameType.PVE})
    ...
    async def test_on_empty_move(self, _, __, ___):
        ...
        self.sio.emit.assert_called_with("error", {"cause": "Missing fields"}, room=self.pve)

        await self.game_handler.on_start(self.pvp, {"rank": 70, "time": 300, "type": GameType.PVP})
        await self.game_handler.on_start(self.pvp1, {"rank": 30, "time": 300, "type": GameType.PVP})
        await self.game_handler.on_move(self.pvp1, {"type": GameType.PVP, "id": "a"*16})
        self.sio.emit.assert_called_with("error", {"cause": "Missing fields"}, room=self.pvp1)

        #empty value test
        await self.game_handler.on_start(self.pve, {"rank": 70, "depth": 1, "time": 300, "type": GameType.PVE})
        await self.game_handler.on_move(self.pve, {"type": GameType.PVE, "san": None})
        self.sio.emit.assert_called_with("error", {"cause": "Encountered None value"}, room=self.pve)

        await self.game_handler.on_start(self.pvp, {"rank": 70, "time": 300, "type": GameType.PVP})
        await self.game_handler.on_start(self.pvp1, {"rank": 30, "time": 300, "type": GameType.PVP})
        await self.game_handler.on_move(self.pvp1, {"type": GameType.PVP, "id": "a"*16, "san": None})
        self.sio.emit.assert_called_with("error", {"cause": "Encountered None value"}, room=self.pvp1)
    ...
    async def test_on_invalid_move(self, _, __, ___):
        await self.game_handler.on_start(self.pve, {"rank": 70, "depth": 1, "time": 300, "type": GameType.PVE})
        await self.game_handler.on_move(self.pve, {"type": GameType.PVE, "san": "hello"})
        self.sio.emit.assert_called_with("error", {"cause": "Invalid move"}, room=self.pve)

        await self.game_handler.on_start(self.pvp, {"rank": 70, "time": 300, "type": GameType.PVP})
        await self.game_handler.on_start(self.pvp1, {"rank": 30, "time": 300, "type": GameType.PVP})
        await self.game_handler.on_move(self.pvp1, {"type": GameType.PVP, "id": "a"*16, "san": "hello"})
        self.sio.emit.assert_called_with("error", {"cause": "Invalid move"}, room=self.pvp1)
    ...
    async def test_on_valid_uci_move(self, mock_play, _, __, ___):
        ...
        await self.game_handler.on_move(self.pvp, {"type": GameType.PVP, "id": "a"*16, "san": "e2e4"})
        self.sio.emit.assert_called_with('error', {'cause': "It's not your turn"}, room=self.pvp)
        #testing moves
        await self.game_handler.on_move(self.pvp1, {"type": GameType.PVP, "id": "a"*16, "san": "e2e3"})
        self.sio.emit.assert_called_with("move", {"san": "e3"}, room=self.pvp)
        await self.game_handler.on_move(self.pvp, {"type": GameType.PVP, "id": "a"*16, "san": "e7e5"})
    ...
    async def test_on_invalid_uci_move(self, mock_play, _, __, ___):
        ...
        #testing moves
        await self.game_handler.on_move(self.pvp1, {"type": GameType.PVP, "id": "a"*16, "san": "e2"})
        self.sio.emit.assert_called_with("error", {"cause": "Invalid move"}, room=self.pvp1)

    @patch('random.randint', return_value=1)
    @patch('random.choice', return_value="a")
    @patch("Game.confighandler.gen_start_fen", return_value='nrk4n/ppp1pppp/8/8/8/1q1bb2r/3K4/8 w - - 0 1')
    @patch.object(chess.engine.UciProtocol, "play")
    async def test_on_checkmate(self, mock_play, _, __, ___):
        ...
        #pvp1 dovrebbe esssere il primo
        #testing for sending message not in turn
        await self.game_handler.on_move(self.pvp, {"type": GameType.PVP, "id": "a"*16, "san": "Ke1"})
        self.sio.emit.assert_called_with('error', {'cause': "It's not your turn"}, room=self.pvp)
        #testing moves
    ...
    @patch('random.randint', return_value=1)
    @patch('random.choice', return_value="a")
    @patch("Game.confighandler.gen_start_fen", return_value='4k3/2Q5/3RBB2/7P/8/2P5/PPP1PPP1/RN2K1N1 w Q - 0 1')
    @patch.object(chess.engine.UciProtocol, "play")
    async def test_on_checkmate_move2(self, mock_play, _, __, ___):
        ...
        #pvp1 dovrebbe esssere il primo
        #testing for sending message not in turn
        await self.game_handler.on_move(self.pvp, {"type": GameType.PVP, "id": "a"*16, "san": "Rd8"})
        self.sio.emit.assert_called_with('error', {'cause': "It's not your turn"}, room=self.pvp)
        #testing moves
    ...
    @patch('random.randint', return_value=1)
    @patch('random.choice', return_value="a")
    @patch("Game.confighandler.gen_start_fen", return_value='nrk4n/ppp1pppp/8/8/3b4/1q1b3r/8/2K5 w - - 0 1')
    @patch.object(chess.engine.UciProtocol, "play")
    ...
    @patch('random.randint', return_value=1)
    @patch('random.choice', return_value="a")
    @patch("Game.confighandler.gen_start_fen", return_value='rnbqkbnr/pppppppp/8/8/5P2/8/PPPPP1PP/RNBQKBNR w KQkq - 0 1')
    @patch.object(chess.engine.UciProtocol, "play")
    async def test_on_enpassant(self, mock_play, _, __, ___):
        ...
        self.sio.emit.assert_called_with('error', {'cause': "It's not your turn"}, room=self.pvp)
        #testing moves
        await self.game_handler.on_move(self.pvp1, {"type": GameType.PVP, "id": "a"*16, "san": "f5"})
        self.sio.emit.assert_called_with("move", {"san": "f5"}, room=self.pvp)
        await self.game_handler.on_move(self.pvp, {"type": GameType.PVP, "id": "a"*16, "san": "e7e5"})
        self.sio.emit.assert_called_with("move", {"san": "e5"}, room=self.pvp1)
        await self.game_handler.on_move(self.pvp1, {"type": GameType.PVP, "id": "a"*16, "san": "fxe6"})
        self.sio.emit.assert_called_with("move", {"san": "fxe6"}, room=self.pvp)
        await self.game_handler.on_move(self.pvp, {"type": GameType.PVP, "id": "a"*16, "san": "d7e6"})
        self.sio.emit.assert_called_with("move", {"san": "dxe6"}, room=self.pvp1)

    @patch('random.randint', return_value=1)
    @patch('random.choice', return_value="a")
    @patch("Game.confighandler.gen_start_fen", return_value='rnbqkbnr/ppPp2pp/8/8/8/8/PPPPP1PP/RNBQKBNR w KQkq - 0 1')
    @patch.object(chess.engine.UciProtocol, "play")
    async def test_on_promotion(self, mock_play, _, __, ___):
        await self.game_handler.on_start(self.pve, {"rank": 70, "depth": 1, "time": 300, "type": GameType.PVE})
        mock_bot_move("e8d8", mock_play)
        await self.game_handler.on_move(self.pve, {"type": GameType.PVE, "san": "cxd8=R+"})
        ...
        self.sio.emit.assert_called_with('error', {'cause': "It's not your turn"}, room=self.pvp)
        #testing moves
        await self.game_handler.on_move(self.pvp1, {"type": GameType.PVP, "id": "a"*16, "san": "cxd8=R+"})
        self.sio.emit.assert_called_with("move", {"san": "cxd8=R+"}, room=self.pvp)
        await self.game_handler.on_move(self.pvp, {"type": GameType.PVP, "id": "a"*16, "san": "e8d8"})
        self.sio.emit.assert_called_with("move", {"san": "Kxd8"}, room=self.pvp1)

    @patch('random.randint', return_value=1)
    @patch('random.choice', return_value="a")
    @patch("Game.confighandler.gen_start_fen", return_value='rnbqkbnr/pppppppp/8/8/2N5/PPNPBPP1/1B1QP2P/R3K2R w KQkq - 0 1')
    @patch.object(chess.engine.UciProtocol, "play")
    async def test_on_castling(self, mock_play, _, __, ___):
        ...
        self.sio.emit.assert_called_with('error', {'cause': "It's not your turn"}, room=self.pvp)
        #testing moves
        await self.game_handler.on_move(self.pvp1, {"type": GameType.PVP, "id": "a"*16, "san": "O-O"})
