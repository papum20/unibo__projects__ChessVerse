...
class PVEGame(Game):
    ...
    def get_current_user():
        Game.cursor.execute(
            "SELECT username FROM backend_registeredusers WHERE session_id = %s",
            (sessionId,),
        )
    ...
    async def move(self, sid: str, data: dict[str, str]) -> None:
        ...
        if outcome is not None:
            ...
            if self.type == 2:
                ...
                if attempts == 0:
                    Game.cursor.execute(
                        "INSERT INTO backend_dailyleaderboard (username,  moves_count, challenge_date, result, attempts) VALUES (%s, %s, %s, %s)",
                        (
                            current_username,
                ...
                if result is None:
                    ...
                    if self.current.move_count < result[0] or (
                        (result[1] >= start_of_week and result[1] <= end_of_week)
                        == False
                    ):
                        Game.cursor.execute(
                            """
